...
def obtener_un_producto(id_producto):
    """NUEVA FUNCION: Devuelve 3 datos de un producto específico o None si no existe."""
    conexion = conectar()
    cursor = conexion.cursor()
    # Usamos fetchone() para traer 1 sola tupla y evitar el ValueError
    cursor.execute("SELECT nombre_producto, cantidad, estado_stock FROM inventario WHERE id_producto=?", (int(id_producto),))
...
def actualizar_estado_producto(id_producto, nuevo_estado):
    ...
    conexion = conectar()
    cursor = conexion.cursor()
    cursor.execute(""" UPDATE inventario SET estado_stock = ? WHERE id_producto = ?""", (nuevo_estado, int(id_producto)))
    conexion.commit()
    conexion.close()
...
def actualizar_producto(id_producto, nombre_nuevo, cantidad_nueva, estado_nuevo):
    ...
    cursor.execute(""" UPDATE inventario SET nombre_producto = ?, cantidad = ?, estado_stock = ? WHERE id_producto =? """, (nombre_final, cantidad_final, estado_final, int(id_producto)))
    conexion.commit()
    conexion.close()
    return True