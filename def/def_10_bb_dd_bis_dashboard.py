...
def reporte_dashboard_unificado():
    ...
    # 1. Recolectamos todos los números sueltos
    cursor.execute("SELECT MAX(puntaje) FROM ranking")
    maximo = cursor.fetchone()[0]

    cursor.execute("SELECT MIN(puntaje) FROM ranking")
    minimo = cursor.fetchone()[0]

    cursor.execute("SELECT AVG(puntaje) FROM ranking")
    # Redondeamos el promedio a 2 decimales para que quede prolijo
    promedio = round(cursor.fetchone()[0], 2) 

    cursor.execute("SELECT COUNT(*) FROM ranking WHERE puntaje = (SELECT MAX(puntaje) FROM ranking)")
    empates_max = cursor.fetchone()[0]

    # 2. Armamos UNA sola tabla con el resumen global
    reporte_encabezado("📊 DASHBOARD GLOBAL DE PUNTAJES")
    
    # Creamos una lista de listas armando las filas a mano
    resumen_estadistico = [
        ["🥇 Puntaje Máximo", maximo],
        ["🥉 Puntaje Mínimo", minimo],
        ["📈 Promedio General", promedio],
        ["🏆 Empates en 1er Puesto", empates_max]
    ]
    print(tabulate(resumen_estadistico, headers=["Métrica", "Valor"], tablefmt="grid"))
    
    cursor.execute(""" SELECT nombre_jugador,puntaje FROM ranking WHERE puntaje >= 800 ORDER BY puntaje DESC """)
